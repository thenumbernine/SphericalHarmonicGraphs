...
do
	...
	vtx = function(l,m,i,j)
		...
		local re, im = sphericalHarmonic[l][m](theta, phi)
		local r = math.abs(re)	
...
do
	...
	normal = function(l,m,i,j)
		...
		local ip = vec3(select(3, vtx(l,m,i+1,j)))
		local im = vec3(select(3, vtx(l,m,i-1,j)))
		local jp = vec3(select(3, vtx(l,m,i,j+1)))
		local jm = vec3(select(3, vtx(l,m,i,j-1)))
	...
	if not list then
		...
		for l=0,lmax do
			for m=-l,l do
				...
				gl.glPushMatrix()
				gl.glTranslatef(l-lmax/2,m,0)
				--gl.glPolygonMode(gl.GL_FRONT_AND_BACK, gl.GL_LINE)
				for i=0,idiv-1 do
					gl.glBegin(gl.GL_TRIANGLE_STRIP)
					for j=0,jdiv do
						for iofs=0,1 do
							local re, im, x, y, z = vtx(l,m,i+iofs,j)
							if re >= 0 then
								gl.glColor3f(1,0,0)
							else
								gl.glColor3f(0,1,1)
